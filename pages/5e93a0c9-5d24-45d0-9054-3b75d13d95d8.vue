<template>
    ...
    <div class="container mx-auto my-12 px-2 sm:px-none">
        <h2>{{ t(2) }}</h2>
        <div class="not-prose">
            <el-text tag="p" class="!mb-2">{{ t(3) }}</el-text>
            <el-text tag="p" class="!mb-2">
                <i18n-t keypath="4">
            ...
            <el-text class="!mb-2">{{ t(10) }}</el-text>
        </div>
        <div class="not-prose my-4"><el-button type="danger" size="large" tag="router-link" to="/medicine/about/">{{
            t(11)
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 not-prose">
            <UseElementVisibility v-slot="{ isVisible }" v-for="({ icon, to }, i) in all" class="text-center">
    ...
    <el-carousel height="80dvh" arrow="always" autoplay class="my-28" indicator-position="none" un-cloak>
        <el-carousel-item v-for="{ images: [{ url }], title, description, icon, to } in the.children"
    ...
    <div class="container mx-auto grid gap-12 my-24 grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 px-2 lg:px-none" un-cloak>
        <UseElementVisibility v-slot="{ isVisible }" v-for="(child, i) in services.$children" class="not-prose flex">
            <el-card class="animate__animated animate__faster"
                ...
                <div @mouseover="slide[i] = true" @mouseleave="slide[i] = false"
                    ...
                    <transition enter-active-class="animate__animated animate__slideInUp animate__faster"
                        ...
                        <router-link v-show="slide[i]" class="bg-white/85 flex-auto flex justify-center items-center"
                            :to="child.to">
...
const { id } = defineProps(["id"]),
    pages = inject("pages"),
    the = pages[id],
    all = Object.values(pages).filter(({ children, enabled }) => enabled && children.length === 0),
    ...
    { t } = useI18n({
        messages: {
            en: {
                0: the.title,
                1: "WE WILL HELP YOU NAVIGATE THE WORLD OF MEDICINE. SINCE 2013, WE HAVE BEEN COORDINATING MEDICAL SERVICES (DIAGNOSTICS, TREATMENT, REHABILITATION). BASED ON YOUR REQUIREMENTS, WE WILL SELECT HIGHLY QUALIFIED DOCTORS AND ORGANIZE MEDICAL SUPPORT FOR YOU.",
                ...
                11: "about us",
                all: [
                    "ABOUT THE COMPANY \"VSYA MEDITSINA\"",
                    "TREATMENT AND REHABILITATION OF PATIENTS WITH COVID-19",
                    "MEDICAL EXAMINATION AND DIAGNOSTICS IN MOSCOW",
                    "HOSPITALIZATION IN MOSCOW",
                    "ASSISTANCE FOR PREVIOUSLY HOSPITALIZED PATIENTS",
